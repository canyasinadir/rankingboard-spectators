			...
			local a = guiGetAlpha(elem) * 255
				...
				if getElementData(playerLabel[id], "spectated") ~= nil then
					if getElementData(playerLabel[id], "spectated") then
						dxDrawImage(2,y,16,16,"img/eye.png", 0,0,0,tocolor(255,255,255,255), false)
					end
				end
			end
		...
		if guiGetVisible(elem) and string.len(guiGetText(elem)) <= 4 then
			...
			if not getKeyState("tab") then
				if getElementData(posLabel[id],"spectated") == true then
					...
					dxDrawText(guiGetText(elem), 0,y,40,y+20, tocolor(0,191,255,a), 1, rankingFont, "right", "center", false,false,false)
				else
					dxDrawText(guiGetText(elem), 1,y+1,41,y+21, tocolor(0,0,0,math.floor(a*0.8)), 1, rankingFont, "right", "center", false,false,false)
